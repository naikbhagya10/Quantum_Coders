...
@token_required
def get_full_medical_history(current_user_id):
    ...
    reports = reports_col.find({'user_id': current_user_id})
    symptoms = symptoms_col.find({'user_id': current_user_id})
    prescriptions = rx_col.find({'user_id': current_user_id})
    appointments = appointments_col.find({'user_id': current_user_id})

    # Historical Biomarker trend points for visualization
    biomarker_trends = [
        ...
        {"date": "2026-07-28", "blood_sugar": 118, "hemoglobin": 12.2, "cholesterol": 195, "systolic_bp": 120}
    ]
